...
class WeightedObjectiveGaussianDiffusion(GaussianDiffusion):
    def __init__(
        ...
        **kwargs
        ...
        assert model.out_dim == (
            channels * 2 + 2
        ), "dimension out (out_dim) of unet must be twice the number of channels + 2 (for the softmax weighted sum) - for channels of 3, this should be (3 * 2) + 2 = 8"
    ...
    def p_losses(self, x_start, t, noise=None, clip_denoised=False):
        ...
        x_start_loss = (
            self.loss_fn(x_start, pred_x_start) * self.pred_x_start_loss_weight
        )
